...
- [-] migration command parser/runner의 `status/up/rollback` 계약과 SQLite 실행, 명시적 migration provider registry, metadata migration 생성과 schema diff/check을 추가했다. 실제 `db` CLI wiring, fixture/seed, admin CLI는 남아 있다.
