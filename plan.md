...
- [-] `nimcrypto` PBKDF2-HMAC-SHA256 password hashing adapter와 per-password salt/parameter encoding, work-factor rehash 판단, stateless signed reset token/expiry 검증을 제공한다. Argon2id/bcrypt, one-time token store, rotation, throttling hook은 후속 범위다.
