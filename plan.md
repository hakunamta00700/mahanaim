...
- [-] background job에 `IdempotencyStore`/in-memory·append-only file·SQLite claim-release adapter와 `enqueueIdempotent`를 추가했다. durable job payload, crash recovery와 외부 queue adapter는 남아 있다.
