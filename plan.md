...
- [x] **P3-20 Redis ChannelLayer lifecycle** — `reconnectWithRetry`가 active group을 재구독하고 `shutdown`이 UNSUBSCRIBE acknowledgement를 drain한 뒤 socket을 닫도록 구현했으며, slow broker loopback과 Redis 7.2.15 live contract로 검증했다. 별도 OS 프로세스 rolling deployment evidence는 P3-17에 남긴다.
