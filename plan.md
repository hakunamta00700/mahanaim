...
- [-] background job에 `IdempotencyStore`/in-memory·append-only file claim-release adapter와 `enqueueIdempotent`를 추가했다. multi-process atomicity, durable job payload, crash recovery와 외부 queue adapter는 남아 있다.
