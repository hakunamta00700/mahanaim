...
- `RedisChannelLayer.registerRedisChannelMetrics`로 Redis delivery snapshot을 애플리케이션 observability endpoint에 연결했다.
- Docker multi-stage/non-root runtime, Nginx TLS reverse proxy, docker-compose와 systemd graceful shutdown 배포 템플릿을 추가했다.
