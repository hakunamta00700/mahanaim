...
- [-] benchmark를 HTTP, router, serialization, ORM query, template rendering 경계로 분리한다. router와 password hash benchmark 및 `databaseQueryBenchmark` ORM query compiler gate를 추가했고, serialization/template/HTTP workload와 버전별 release artifact 기록은 후속 범위다.
