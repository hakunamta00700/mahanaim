...
- [-] bound query compiler 위에 공통 pagination/filter/sort/field-selection component와 immutable-style QuerySet builder, metadata-driven aggregate expression parser, grouped aggregate SQL compiler/result mapping, typed arithmetic annotate projection, eager one-hop loading과 명시적 lazy relation loader를 연결했다. many-to-many through loading은 후속 범위다.
...
| [-] | REQ-DATA-002 | P1 | QuerySet/query builder AST와 metadata-driven aggregate parser로 조건·정렬·pagination·grouping·aggregate SQL, typed arithmetic annotate projection, eager one-hop loading, 명시적 lazy relation loader 및 repository JSON result mapping을 제공한다. many-to-many through loading은 남아 있다. |
