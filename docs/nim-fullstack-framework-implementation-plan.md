...
- [-] migration은 빈 DB, 기존 데이터, rollback, 동시 요청 조건에서 검증한다. SQLite unit fixture가 독립 연결의 concurrent migrate, idempotent history, rollback을 검증하며 PostgreSQL concurrent migration과 staging 운영 evidence는 별도 범위다.
