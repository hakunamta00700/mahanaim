...
- [-] 다중 operation registry와 Swagger/ReDoc UI route를 추가했고 scalar object용 `inputSchema`/`responseSchema` macro가 `FieldSpec`를 생성한다. registry 기반 nested DTO OpenAPI `$ref`/cycle schema와 router 기반 idempotent `collectRoutes`를 추가했으며 자동 body schema 추론과 완전한 content negotiation은 남아 있다.
...
- [-] 수동 route/schema registry에서 OpenAPI 3.1을 만들고 Swagger UI·ReDoc route를 제공한다. registry 기반 nested DTO component schema, cycle-safe `$ref`, router 기반 idempotent `collectRoutes`를 추가했으며 자동 body schema 추론은 남아 있다.
...
- [-] 사용자·그룹·role·permission·route guard·object-level authorization을 `AuthorizationPolicy`로 구현했고 algorithm-neutral `PasswordHasher` 계약, PBKDF2 reference adapter, work-factor rehash 판단, stateless signed reset token, atomic one-time token store, password rotation, 교체 가능한 login throttle 계약과 session key rotation을 추가했다. adapter-neutral account store와 login/logout/password-change/password-reset request·confirm route flow도 추가했으며 실제 Argon2id/bcrypt 구현 adapter는 남아 있다.
...
| [-] | REQ-API-004 | P1 | route/schema registry에서 OpenAPI 3.1을 생성하고 Swagger UI·ReDoc route 및 `addDocumentedRoute` 동시 등록 경계를 붙였다. scalar input/response object용 `inputSchema`·`responseSchema` macro, registry 기반 nested DTO component schema/cycle-safe `$ref`, router 기반 idempotent `collectRoutes`를 추가했으며 자동 body schema 추론은 남아 있다. |
...
| 상태 | ID | 우선순위 | 구현 계획 |
| --- | --- | --- | --- |
| [-] | REQ-UI-001 | P1 | auto-escaping renderer와 inheritance·partial(include)·filter registry, locale catalog 기반 translation helper를 구현했다. AST 기반 확장 helper와 locale 파일 자동 로딩은 후속 범위다. |
...
- [-] **P1-02**: OpenAPI registry generator, Swagger UI, ReDoc route, nested DTO component `$ref`, router 기반 idempotent `collectRoutes`를 추가했고 자동 body schema 추론은 남아 있다.
