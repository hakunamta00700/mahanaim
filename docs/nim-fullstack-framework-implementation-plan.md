...
- [-] `IdempotencyStore` claim/release contract, in-memory reference adapter와 `enqueueIdempotent` 중복 억제 경계를 추가했다. durable job persistence, crash recovery와 외부 queue adapter는 남아 있다.
...
- [-] 작업별 idempotency key claim/release와 bounded retry 경계를 추가했다. durable 외부 queue adapter와 crash recovery는 남아 있다.
