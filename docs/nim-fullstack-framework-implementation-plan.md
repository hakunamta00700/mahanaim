...
- [-] migration command parser/runner의 `status/up/rollback` 계약과 SQLite 실행, 명시적 registry 로딩을 제공했다. Application-aware `db status|up|rollback` CLI wiring까지 연결했으며, 생성·검토, schema diff/check, fixture/seed는 남아 있다.
...
- [-] model metadata에서 validation `FieldSpec`, `bindModelForm`, OpenAPI schema와 field widget registry를 생성하는 bridge를 추가했다. model formset은 남아 있다.
...
| [-] | REQ-DATA-003 | P1 | SQLite migration artifact와 command runner의 up/down/status, 명시적 provider registry, Application-aware `db status|up|rollback` CLI, metadata migration 생성과 schema diff/check을 제공한다. fixture/seed는 남아 있다. |
