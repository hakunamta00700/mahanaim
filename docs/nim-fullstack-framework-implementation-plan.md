...
- [x] 동적 nested collection projection과 AST-aware `TemplateHelperArgument`/`registerHelper`를 추가하고, `TemplateNode` 구조형 AST parser/render로 현재 loop context·named argument·quoted literal을 안전하게 렌더링하며 교차 종료 태그를 거부하는 회귀 테스트를 추가했다.
...
- [-] embedding `runCli`와 standalone CLI가 `dev`, `db migrate`/`up`, `admin create-user`, `static collect`, `test`, `check`, `jobs` 명령의 공통 계약을 사용하도록 연결했다. `mahanaim new`가 생성한 애플리케이션 모듈은 `createApp()`과 `commandLineParams()`를 공통 `runCli`에 명시적으로 연결하고, app-owned migration registry와 admin provisioning callback도 구성한다. 임의 모듈 자동 import는 하지 않으며, persistence/credential 정책은 생성 프로젝트가 소유한다.
...
- [x] 다중 operation registry와 Swagger/ReDoc UI route, operation별 다중 request/response content type을 추가했고 scalar object용 `inputSchema`/`responseSchema` macro와 `addTypedDocumentedRoute`가 `FieldSpec`를 생성한다. registry 기반 nested DTO OpenAPI `$ref`/cycle schema, router 기반 idempotent `collectRoutes`, metadata 기반 `addModelDocumentedRoute`와 `Application.dispatch` 공통 runtime content negotiation을 추가했다.
- [-] type-erased generic handler closure의 DTO body schema 자동 추론은 handler/DTO 소유권과 framework-neutral 경계를 흐릴 수 있으므로 지원하지 않는다. 별도 typed handler contract가 정의될 때까지 명시적 `addTypedDocumentedRoute` 선언을 사용한다.
...
- [x] template inheritance/include/filter의 독립 엔진, `TemplateNode` 구조형 AST, 명시적 `TemplateRenderContext` collection loop, 현재 loop context 기반 `TemplateCollectionProjection`, AST-aware `registerHelper`, metadata 기반 model formset, locale별 JSON catalog directory loader를 구현하고 회귀 테스트를 추가했다.
...
- [-] 공통 query component를 CRUD/admin list의 in-memory reference adapter까지 연결하고 QuerySet aggregate SQL compiler, repository JSON result mapping, explicit aggregate route adapter를 추가했다. typed cursor token/bound filter, signed/expiring next cursor와 opt-in count/total metadata를 추가했다.
...
- [x] command/admin extension의 lifecycle integration을 startup 전용 registration window와 startup transition guard로 고정했다. late command/admin/plugin/hook 및 runtime request-surface mutation은 contract regression test로 검증하며, 실제 외부 deployment evidence는 application-owned 범위다.
...
- [x] 실제 RESP/network client, server TTL 파싱과 loopback live socket fixture를 추가했고 reconnect와 in-memory bounded eviction을 검증했다. TCP coalescing frame buffer, 환경 기반 `redisLive` contract, `INFO server`, `CONFIG GET`, `COMMAND INFO` 기반 Redis/Valkey flavor/version/필수 command/maxmemory/eviction probe를 추가했으며 Linux Nim 2.2.4 matrix에서 Redis 7.2.15와 Valkey 8.1.9의 실제 PING·command·bounded eviction·server-side TTL을 통과시켰다. S3 signing/retry와 별도 cache eviction 부하 운영은 남아 있다.
...
- [-] 작업별 idempotency key claim/release와 bounded retry 경계를 추가하고 file journal/SQLite unique constraint로 restart 및 multi-connection key 상태를 보존한다. SQLite durable job payload와 processing recovery, named handler executor dispatch도 제공하며 외부 queue adapter는 남아 있다.
...
- [x] provider wiring contract test를 추가했다. 생성 앱 모듈은 `createApp()`을 사용하는 명시적 standalone CLI 진입점으로 생성된다.
- [x] standalone의 app-owned migration/account callback wiring과 생성기 fixture를 추가했다. 생성 앱은 migration registry와 admin creator를 명시적으로 구성하며, 저장소와 credential 정책은 프레임워크가 추측하지 않고 생성 프로젝트가 소유한다.
...
- [x] 안전한 기본 escaping, `TemplateNode` 구조형 AST 기반 inheritance, include/partial, filter registry, nested `if/else/endif`와 `for` block, legacy `registerTag` 및 named/quoted/context argument를 받는 AST-aware `registerHelper`를 갖춘 독립 template engine을 추가했고 locale catalog 기반 `registerTranslation`/`translate` helper와 JSON catalog file loading, 명시적 `TemplateRenderContext` collection loop 및 현재 context 기반 동적 `TemplateCollectionProjection`을 제공한다. `Request.locale`/`Request.timezoneOffsetMinutes`와 Accept-Language middleware, 명시적 timezone offset 및 `timezones` 기반 IANA/DST 날짜·시간, locale 숫자 formatter도 제공한다.
...
| [x] | REQ-DATA-003 | P1 | SQLite migration artifact와 command runner의 up/down/status, PostgreSQL migration history runner와 shared command overload, 명시적 `MigrationDatabaseProvider`, Application-aware `db status|up|rollback` 및 atomic `db seed` CLI, metadata migration 생성과 schema diff/check을 제공한다. PostgreSQL 16 live fixture와 provider wiring contract test를 통과했고 생성 앱이 동일한 migration 정의를 Application registry와 standalone CLI에 연결한다. PostgreSQL provider 선택과 CI fixture wiring은 application-owned extension 범위로 분리한다. |
...
| [-] | REQ-SEC-003 | P1 | algorithm-neutral `PasswordHasher` 계약과 표준 PBKDF2-HMAC-SHA256 reference adapter, C-backed Argon2id adapter, Nim maintained pure bcrypt adapter, per-password salt/parameter encoding, work-factor 판단·`verifyAndRehash` rotation, current-password 검증 기반 `changePassword`, stateless signed reset token/expiry 검증, atomic one-time reset token store, 교체 가능한 login throttling hook과 in-memory·distributed counter adapter, adapter-neutral account store와 login/logout/password-change/password-reset request·confirm route flow, Argon2/bcrypt hash·verify benchmark harness와 Windows/Linux CI contract를 제공했다. 2026-08-05 Windows 개발 호스트 release-like baseline(Argon2id 117/119 ms, bcrypt work factor 12 249/250 ms)을 운영 문서에 기록했으며, 실제 production benchmark와 concurrent load 기반 cost 확정은 남아 있다. |
...
| [-] | REQ-SEC-006 | P2 | rate limit·size·timeout·secure cookie 정책과 HTTPS reverse-proxy 배포 점검표를 문서화하고, `requireHttps`와 직접 peer 기반 `trustedProxies`로 forwarded scheme/host contract 및 회귀 테스트를 추가했다. `checkApplication`의 공개 host 미고정 warning도 연결했다. Docker nginx TLS 1.2/1.3 → Nim 2.2.4 upstream live wire와 외부 endpoint용 `httpsLive` client는 통과했으며 운영 staging 인증서/renewal·redirect 검증은 남아 있다. |

### 운영·확장·검증

| 상태 | ID | 우선순위 | 구현 계획 |
| --- | --- | --- | --- |
| [-] | REQ-OPS-001 | P2 | `static collect`와 upload local filesystem contract, backend-neutral `ObjectStorage`/`CacheStore`, bounded in-memory object/cache adapter와 S3-compatible transport bridge, Redis/Valkey RESP rate-limit·cache 구현체와 오류/재시도 경계를 추가하고 request/success/failure/connection/reconnect snapshot metrics, TCP coalescing frame buffer, `INFO`/`CONFIG`/`COMMAND INFO` compatibility probe와 환경 기반 live contract, in-memory monotonic TTL·bounded eviction을 제공했다. Redis 7.2.15/Valkey 8.1.9 matrix는 통과했으며 S3 signing/retry와 별도 cache eviction 부하 운영 정책은 남아 있다. |
| [-] | REQ-OPS-002 | P2 | task contract와 bounded retry, `IdempotencyStore` claim/release, `enqueueIdempotent`, SQLite durable payload state machine, processing recovery와 named-kind executor dispatch, `ExternalDurableJobStore` callback bridge를 제공했다. SQLite와 external store의 모든 close 이후 state transition은 명시적 lifecycle 오류를 반환하고 close callback은 한 번만 실행하며 shutdown race/idempotency 회귀 테스트로 검증한다. 실제 외부 provider protocol·visibility timeout·ack 운영 검증은 application-owned 범위다. |
...
| [-] | REQ-OPS-004 | P2 | email·flash·RSS/Atom·sitemap을 서버 렌더링용 독립 패키지로 제공한다. session key별 bounded FIFO·consume-once `FlashStore`, XML-escaped sitemap/RSS 2.0·Atom 1.0 renderer, RFC 5322 simple-part `EmailMessage` serializer와 `EmailTransport`/in-memory·wire callback adapter를 추가했다. 외부 SMTP socket·credential·retry provider 운영 검증은 application-owned 범위로 남아 있다. |
| [-] | REQ-OPS-005 | P2 | `Request.locale`/`Request.timezoneOffsetMinutes` 협상과 명시적 timezone offset 및 `timezones` 기반 IANA/DST 날짜·시간·숫자 formatter를 공통 경계로 제공했다. `TemplateRenderContext`가 request-owned formatter snapshot과 `format_decimal`·`format_datetime` built-in helper를 자동 주입하며 unconfigured/invalid input/reserved name 회귀를 검증한다. form/API formatter 자동 주입과 고급 formatting은 남아 있다. |
...
- [-] 모든 보안 기본값은 성공 케이스보다 실패·우회 케이스를 먼저 테스트한다. HTTPS/Host/CORS/body-limit/CSRF/session/rate-limit 실패 계약과 untrusted forwarded host·missing Host 우회 회귀를 `tests/test_core.nim`에서 검증했으며, 추가 adapter/live 운영 증거는 별도 범위다.
- [-] migration은 빈 DB, 기존 데이터, rollback, 동시 요청 조건에서 검증한다. SQLite unit fixture와 PostgreSQL live fixture가 독립 연결의 concurrent migrate, idempotent history, rollback을 검증하며, PostgreSQL credential 환경에서의 실제 실행 로그와 staging 운영 evidence는 별도 범위다.
...
- [ ] reconnect·ordering·backpressure와 실제 Redis/Valkey cross-process fan-out live contract는 외부 서비스 환경 범위로 남긴다. Dedicated async subscription socket은 아래 contract로 완료했다.
